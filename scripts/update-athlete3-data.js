...
// Athlete 3 updated biometric data with additional fields
const athlete3Updates = [
  {
    date: '2025-08-31',
    restingHeartRate: 55,
...
async function updateAthlete3Data() {
  console.log('🔄 Starting to update biometric data for Athlete 3 with additional fields...');

  for (const record of athlete3Updates) {
    try {
      const payload = {
      ...
      console.log(`📤 Updating data for ${record.date}...`);
      const response = await axios.post(`${API_BASE_URL}/athletes/3/biometric-data`, payload);

      if (response.status === 200 || response.status === 201) {
        console.log(`✅ Successfully updated data for ${record.date}`);
      }
    } catch (error) {
      console.error(`❌ Failed to update data for ${record.date}:`, error.response?.data || error.message);
    }
  ...
  console.log('🎉 Finished updating biometric data for Athlete 3!');
  console.log('🔄 Refresh the athlete profile page to see the updated metrics in charts.');
}

updateAthlete3Data().catch(console.error);