...
import { dataService, biometricDataService } from '../services/dataService';
...
const HeartRateTrendChart: React.FC<HeartRateTrendChartProps> = ({ athleteId }) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const biometricData = await biometricDataService.getBiometricDataByAthlete(athleteIdNum);
      ...
      const filteredResults = biometricData.filter(
      ...
      if (error.response?.status === 401 || error.name === 'AuthError') {
        ...
        toast.error('Failed to load heart rate trend data. Please try again.');
